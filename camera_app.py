...
class CameraApp(tk.Tk):
    def __init__(self):
        ...
        self.btn_record = tk.Button(
            self.control_frame, text="Record Video", command=self.toggle_record
        )
        self.btn_record.pack(side=tk.LEFT, padx=10)
        self.btn_gallery = tk.Button(
            self.control_frame, text="Gallery", command=self.open_gallery
        )
        self.btn_gallery.pack(side=tk.LEFT, padx=10)
        self.is_recording = False
        self.video_writer = None
        self.btn_quit = tk.Button(
            self.control_frame, text="Quit", command=self.quit_app
        )
    ...
    def toggle_record(self):
        if not self.is_recording:
            ...
            self.video_writer = cv2.VideoWriter(
                file_name, cv2.VideoWriter_fourcc(*"XVID"), 20, (640, 480)
            )
    ...
    def file_list(self):
        return [
            ...
            if file.startswith("captured_image") or file.startswith("recorded_video")
        ]
...
class GalleryApp(tk.Toplevel):
    def __init__(self, camera_app):
        ...
        self.gallery_title = tk.Label(
            self.gallery_frame, text="Gallery", font=("Helvetica", 24, "bold")
        )
        self.gallery_title.pack(pady=20)
        self.back_button = tk.Button(
            self.gallery_frame, text="Back to Camera", command=self.back_to_camera
        )
        self.back_button.pack(pady=10)
        self.preview_canvas = tk.Canvas(self.gallery_frame, bg="white")
        self.preview_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.scrollbar = ttk.Scrollbar(
            self.gallery_frame, orient=tk.VERTICAL, command=self.preview_canvas.yview
        )
    ...
    def show_previews(self):
        if not self.file_list:
            label = tk.Label(
                self.preview_frame, text="No items found.", font=("Helvetica", 16)
            )
            label.pack()
            return
        for i, file in enumerate(self.file_list):
            if file.startswith("captured_image"):
                ...
                if ret:
                    ...
                    label.grid(row=i // 3, column=i % 3, padx=5, pady=5)
                    label.bind(
                        "<Button-1>", lambda event, file=file: self.open_file(file)
                    )
